...
const UpdateProduct = () => {
    ...
    const onSubmitHandler = async (e) => {
        ...
        try {
            ...
            const response = await axios.put(`${backendUrl}/api/product/update/${productId}`, formData, { withCredentials: true })
        ...
        const fetchProduct = async () => {
            try {
                ...
                if (response.data) {
                    ...
                    quillRef.current.root.innerHTML = response.data[0].about
                    quillRef2.current.root.innerHTML = response.data[0].description
                    setPrice(response.data[0].price)
                    setOfferPrice(response.data[0].offerPrice)
        ...
        <form onSubmit={onSubmitHandler} className='flex-1 px-4 py-8 lg:py-10 lg:px-14 bg-blue-50/50 text-gray-600 h-full min-h-[95vh]'>
            <div className='bg-white flex flex-col w-full max-w-3xl p-6 md:p-10 shadow rounded'>
                <div className='flex items-center'>
                    <label htmlFor="image">
                        <img src={!image ? previewImage : URL.createObjectURL(image)} className='rounded cursor-pointer max-h-24 max-w-24' alt="" />
                ...
                <select defaultValue={category} onChange={(e) => setCategory(e.target.value)} name="category" className='mt-2 w-fit px-3 py-2 text-[13px] bg-[#f6fafd] border text-gray-500 border-gray-300 outline-none rounded'>
                ...
                <button type='submit' className='mt-7 text-sm px-8 w-fit py-[10px] bg-[#994CF5] text-white rounded cursor-pointer'>{loading ? 'Saving...' : 'Save Changes'}</button>
            </div>
        </form>
    )
}
